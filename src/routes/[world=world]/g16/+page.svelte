<script>
	...
	function g16GenPattern(lv){
		...
		const p=/** @type {{seq:any[],ans:any,opts:any[]}} */ (patterns[Math.floor(Math.random()*patterns.length)]);
		const numDist=lv<=3?1:lv<=6?2:3;
		p.opts=[p.ans,...shuf(pool.filter((/** @type {string} */ x)=>x!==p.ans)).slice(0,numDist)];
		return p;
	}
	...
	function initG16(cont, lv) {
		...
		function next(){
			...
			const opEl=/** @type {HTMLElement} */ (cont.querySelector('#g16opts'));opEl.innerHTML='';
