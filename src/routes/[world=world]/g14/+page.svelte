<script>
	...
	function generateMaze(rows,cols){
		...
		function dfs(r,c){
			...
			for(const{dr,dc,wall,opp}of dirs){
				...
				if(nr>=0&&nr<rows&&nc>=0&&nc<cols&&!grid[nr][nc].vis){grid[r][c][wall]=false;grid[nr][nc][opp]=false;dfs(nr,nc);}
			}
		}
		dfs(0,0);return grid;
	}
	...
	function initG14(cont, lv) {
		...
		function render(){
			const el=cont.querySelector('#g14maze');
			...
			for(let r=0;r<rows;r++){
				for(let c=0;c<cols;c++){
					...
					cell.onclick=()=>move(parseInt(cell.dataset.r),parseInt(cell.dataset.c));
					el.appendChild(cell);
				}
			}
		}
		...
		cont.querySelector('#dp-l').onclick=()=>moveDir('left');
		cont.querySelector('#dp-r').onclick=()=>moveDir('right');
