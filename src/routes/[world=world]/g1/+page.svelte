<script>
	...
	function initG1(cont, lv) {
		...
		const shapesEl = container.querySelector('#g1shapes');
		const outsEl = container.querySelector('#g1outs');
		...
		outOrder.forEach(s => {
			...
			el.addEventListener('touchend', e => { e.preventDefault(); if(g1TapSel) g1TapPlace(el, s); });
			el.onclick = () => { if(g1TapSel) g1TapPlace(el, s); };
	...
	function g1FinishDrag(cx, cy) {
		...
		const target = under?.closest('.g1-out');
		if (target && target.dataset.id === g1DragEl.dataset.id) {
			...
			if (g1Matched === container.querySelectorAll('.g1-shape').length) {
				const _lv = window.ppWin(); cleanup(); setTimeout(() => window.ppCelebrate('¡Todas las figuras! 🔷', 3, () => initG1(container, window.ppGetLevel()), _lv), 400);
			}
		} else {
	...
	function g1TapSelect(el, shape) {
		...
		container?.querySelectorAll('.g1-shape').forEach(x => { x.style.outline = ''; x.style.boxShadow = ''; });
