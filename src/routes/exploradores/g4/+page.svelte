<script>
	...
	function initG4(cont, lv) {
		...
		const cvs = cont.querySelector('#g4cvs');
		...
		const palEl = cont.querySelector('#g4pal'); palEl.innerHTML = '';
		...
		const toolsEl = cont.querySelector('#g4tools'); toolsEl.innerHTML = '';
		...
		tools.forEach(t => {
			...
			b.onclick = () => {
				...
				if(t.id==='brush'){g4Mode='brush';cont.querySelector('#g4stamps').style.display='none';return;}
				if(t.id==='stamp'){g4Mode='stamp';cont.querySelector('#g4stamps').style.display='flex';return;}
			};
			toolsEl.appendChild(b);
		});

		const stampsEl = cont.querySelector('#g4stamps'); stampsEl.innerHTML = '';
		G4_STMP.forEach(s => {
